...
def preprocess_wine_data(wine_data):
    ...
    weighted_features = np.hstack(
        [wine_data["price_normalized"].values.reshape(-1, 1) * 40, encoded_features]
    )
...
def list_wines(
    variety: str = None, region: str = None, country: str = None, limit: int = 50
):
    """
...
def suggest_wines(
    wine_name: str = Query(
        ..., description="The name of the wine to base suggestions on"
    ),
    ...
    weighted_price = (
        wine_data["price_normalized"].values.reshape(-1, 1) * weights["price"]
    )
    weighted_features = np.hstack(
        (
            weighted_price,
    ...
    review_vectors = np.array(
        [np.array(vec).flatten() for vec in wine_data["review_vector"]]
    )
    ...
    if result_df[numeric_columns].isnull().any().any():
        logger.warning(
            "Detected NaN values in numeric columns. Filling with default values."
        )
    if (result_df[numeric_columns] == np.inf).any().any() or (
        result_df[numeric_columns] == -np.inf
    ).any().any():
        logger.warning(
            "Detected infinite values in numeric columns. Replacing with None."
        )
